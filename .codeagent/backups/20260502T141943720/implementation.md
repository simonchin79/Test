...
### macOS-Specific: Render Loop Fix
- On macOS (Apple Silicon, macOS 26.x), the app uses the **`basic`** render loop
  (`QSG_RENDER_LOOP=basic` set in `main.cpp`) instead of the default threaded loop.
- The threaded render loop (`QSGRenderThread`) crashes with `EXC_BAD_ACCESS`
  (`EXC_ARM_DA_ALIGN`) when rendering emoji glyphs because Apple's CoreText
  `CopyEmojiImage` → ImageIO `PNGReadPlugin` path is not thread-safe.
